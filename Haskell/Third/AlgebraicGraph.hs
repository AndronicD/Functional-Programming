...
splitNode node targets = extend (\x -> if node == x then foldl Overlay Empty (map Node targets) else (Node x))          
...
instance Functor AlgebraicGraph where
    ...
    fmap f (Overlay graph1 graph2) = Overlay (fmap f graph1) (fmap f graph2) 
    fmap f (Connect graph1 graph2) = Connect (fmap f graph1) (fmap f graph2) -} 
...
removeNode node graph = filterGraph (\x -> if x == node then False else True) graph         