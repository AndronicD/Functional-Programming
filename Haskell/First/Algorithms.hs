...
searchAux f graph visited structure = 
    if null structure
        then visited
        else searchAux f graph (visited ++ [head structure]) (f (tail structure) (filter (\x -> notElem x visited && notElem x structure) (S.toList (outNeighbors (head structure) graph))))
...
queue inside toQueue = foldr (\x acc -> if elem x acc then [] ++ acc else [x] ++ acc) inside toQueue 

stack inside toStack = foldr (\x acc -> if elem x acc then acc ++ [] else acc ++ [x]) inside toStack
{-
    *** TODO ***
