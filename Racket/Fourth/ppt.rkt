...
(define (multiply M V)
  (foldr (lambda (x L) (cons (dot-product x V) L)) '() M))
...
(define ppt-stream-in-tree-order
  (let loop ((Q (stream '(3 4 5))))
    (let ((first (stream-first Q)))
     (stream-cons first (loop (stream-rest (stream-append Q (calc first)))))
    )))

(define (calc F)
  (stream (multiply T1 F) (multiply T2 F) (multiply T3 F)))
...
(define (pairs G H)
    (let loop ((g G) (h H) (i 0) (j 1))
      (if (< i j) (stream-cons (cons (stream-first g) (stream-first h)) (loop (stream-rest g) h (+ i 1) j)) (loop G (stream-rest h) 0 (+ j 1)) )))
...
(define gh-pairs-stream
  (let ((g odd-naturals) (h (stream-rest odd-naturals)))
    (stream-filter (lambda (x) (if (= (gcd (cdr x) (car x)) 1) #t #f)) (pairs g h))))

; TODO
; Definiți fluxul de TPP corespunzător fluxului anterior de
; perechi (g, h).2
(define (second_term g h)
  (/ (- (* h h) (* g g)) 2))

(define (third_term g h)
  (/ (+ (* h h) (* g g)) 2))

(define ppt-stream-in-pair-order
  (let loop ((flux gh-pairs-stream))
    (let ((g (car (stream-first flux))) (h (cdr (stream-first flux))))
      (let ((a (* g h)) (b (second_term g h)) (c (third_term g h)))
        (stream-cons (list a b c) (loop (stream-rest flux)))))))