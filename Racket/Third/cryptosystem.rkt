...
(define (key n)
  (let ((e (cadr (get-nth-quadruple n))) (f (caddr (get-nth-quadruple n)))) (map (lambda (x) (modulo x 27)) (create-list e f))))

(define (create-list e f)
  (let ((a1 (a1 e f)) (b1 (b1 e f)) (c1 (c1 e f))
                      (a2 (a2 e f)) (b2 (b2 e f)) (c2 (c2 e f))
                      (a3 (a3 e f)) (b3 (b3 e f)) (c3 (c3 e f)))
...
(define (message->codes message)
  (map (lambda (x) (if (= (char->integer x) 32) 0 (- (char->integer x) 96))) (string->list message)))
...
(define (codes->message codes)
  (list->string (map (lambda (x) (if (= x 0) #\space (integer->char (+ x 96)))) codes)))
...
(define (extend-key key size)
  (extend-key-helper key size '()))

(define (extend-key-helper key size acc)
  (if (= size 0) (reverse acc) (let ((x (car key))) (extend-key-helper (bring-elem-to-back key) (- size 1) (cons x acc)))))

(define (bring-elem-to-back key)
  (let ((x (car key))) (reverse (cons x (reverse (cdr key))))))
  ...
  (lambda (x)
    (lambda (y)
      (lambda (z)
        (lambda (sign)
          (lambda (w t)
            (z (let ((L (x w))) (map (lambda (a1 a2) (modulo (sign a1 a2) 27)) L (y t (length L)))))))))))
...
;;      (lambda (z)
;;        (lambda (w t)
;;          (z (let ((L (x w))) (map (lambda (a1 a2) (modulo (- a1 a2) 27)) L (y t (length L))))))))))
          

(define encrypt-codes
  (curry ((((f (lambda (x) x)) extend-key) (lambda (x) x))+)))
...
(define encrypt-message
  (curry ((((f message->codes) extend-key) codes->message)+)))
